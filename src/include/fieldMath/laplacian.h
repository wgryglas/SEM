...
#include "LaplaceDiscreteOperator.h"
...
inline typename LaplaceDiscreteOperator<T,Coeff>::ref laplacian(const Coeff &c, field::GeometricField<T> &f)
{
    return typename LaplaceDiscreteOperator<T,Coeff>::ref(new LaplaceDiscreteOperator<T,Coeff>(f,c));
}
